class MasterOrdersController < ApplicationController
  ...
  def create
    ...
    sum=0
    @order.each do |o|
      sum = sum + (o.price.to_i)*(o.quantity)
    end
  ...
  def master_order_params
    params.require(:master_order).permit(:total,:order_type,:payment_type,:order_status,:payment_status,:transaction_id,:user_id,:restaurant_id)
  end
end
